        ...
        if num1 > num2:
            if num2 >= 10:
                ...
                if sum >= 100:
                    print("Nope")
                #Checks to make sure only numbers under 100 and the actual solutions are added
                elif sum > 9 and sum % 1 == 0:
                    f = open(r"C:\Users\student\Desktop\Equations\divison.txt", 'a')
                    #Handles formatting based on which number is larger
                    if num1 > num2:
                        sum = int(num1 / num2)
                        if sum < 1000:
                            f.write(str(num1)+ div + str(num2) + equal + str(sum) + "\n")
                    elif num1 < num2:
                        sum = int(num2 / num2)
                        if sum < 1000:
                            f.write(str(num2)+ div + str(num1) + equal + str(sum) + "\n")
        elif num1 < num2:
            if num1 >= 10:
                sum = num2 / num1
                if sum >= 100:
